...
#include "communication/unified_command_convertor_recorder.h"
...
static bool executeShellCommand(){
    ...
    if( ARGS_PARSER.isKeyExist(EPlayerArguments::SHELL_CMD_START_RECORDER_AGENT) ){
        ...
        if( ARGS_PARSER.isKeyExist(EPlayerArguments::AS_DAEMON) ){
            ...
            logger_common::SInitSettings settings2;
            settings2.loggerName = "RecorderAgentDaemon";
            settings2.unilogConfigPath = CONFIG_PARAMS.baseParams.SYSTEM_UNILOG_CONFIG_PATH;
        ...
        settings.loggerName = "RecorderShellClient";
...
int main( int argc, char ** argv, char ** env ){
    ...
    if( ! initSingletons(argc, argv, env) ){
        PRELOG_ERR << "============================ RECORDER START FAILED (singletons area) ============================" << endl;
        return -1;
    }
    ...
    if( ! executeShellCommand() ){
        VS_LOG_ERROR << "============================ RECORDER START FAILED ============================" << endl;
        return -1;
    }
#else
