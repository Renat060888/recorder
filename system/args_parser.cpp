...
void ArgsParser::checkArgumentsTemplateMethodPart( const bpo::variables_map & _varMap ) {
    ...
    if( _varMap.find("start-agent") != _varMap.end() ){
        m_commmandLineArgs[EPlayerArguments::SHELL_CMD_START_RECORDER_AGENT] = "bla-bla";
    }

    if( _varMap.find("start-contoller") != _varMap.end() ){
        m_commmandLineArgs[EPlayerArguments::SHELL_CMD_START_RECORDER_CONTROLLER] = "bla-bla";
    }

    if( _varMap.find("stop") != _varMap.end() ){
        m_commmandLineArgs[EPlayerArguments::SHELL_CMD_TO_RECORDER]
